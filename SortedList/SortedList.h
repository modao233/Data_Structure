...
class SortedList:public SeqList<E> {
public:
...
template <typename E, typename K>
void SortedList<E, K>::Insert(const K k1, E &e1) {
    int i, j;
    for (i = 0; i <= last; i++) {
        if (data[i] == k1) {
            data[i] = e1;
...
template <typename E, typename K>
int SortedList<E, K>::Search(K k1) const {      //顺序查找
    int i;
    for (i = 0; i <= last; i++) {
        if (data[i] == k1) {
            return i + 1;
...
template <typename E, typename K>
int SortedList<E, K>::BinarySearch(K k1, int low, int high) const {     //递归对半查找
    int mid = 0;
    if (low <= high) {
        mid = (low + high) / 2;
        if (data[mid - 1] < k1) {
...
template <typename E, typename K>
int SortedList<E, K>::BinarySearch(K k1) const {        //迭代对半查找
...
template <typename E, typename K>
int SortedList<E, K>::FibonacciSearch(K k1) const {     //斐波那契查找
    int i = 1, j;
    while (Fib(i) < last + 1) {
        i++;
    }
